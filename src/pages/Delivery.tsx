...
import { orders as initialOrders, drivers, orderStatusColors } from '@/data/mockData';
...
const Delivery = () => {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><Truck className="w-6 h-6" /> {t.deliveryPage.title}</h1>
        <p className="text-sm text-muted-foreground">{t.deliveryPage.subtitle}</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4">
            <h3 className="font-semibold text-foreground mb-3">{t.deliveryPage.drivers}</h3>
            {drivers.map(driver => (
              <div key={driver.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                ...
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${driver.isAvailable ? 'bg-status-safe text-status-safe-foreground' : 'bg-status-warning text-status-warning-foreground'}`}>
                  {driver.isAvailable ? t.deliveryPage.available : t.deliveryPage.onRoute}
                </span>
              </div>
            ))}
          </CardContent>
        ...
        <div className="md:col-span-2 space-y-3">
          {deliveryOrders.length === 0 && (
            <Card className="border-0 shadow-sm">
              <CardContent className="p-8 text-center text-muted-foreground">{t.deliveryPage.noDeliveryOrders}</CardContent>
            </Card>
          )}
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <span className={`ms-2 inline-block px-2 py-0.5 rounded-full text-[10px] font-medium ${orderStatusColors[order.status]}`}>{t.orderStatus[order.status as keyof typeof t.orderStatus]}</span>
                    </div>
                    <p className="text-sm font-medium text-foreground">{t.currency} {order.totalAmount.toLocaleString()}</p>
                  </div>
                  <p className="text-sm text-foreground mb-1">{order.clientName}</p>
                  <p className="text-xs text-muted-foreground mb-3 flex items-center gap-1"><MapPin className="w-3 h-3" /> {order.requestedDate} {t.kitchen.at} {order.requestedTime}</p>
                  
                  <div className="flex items-center gap-3">
                    {order.status === 'ready' && (
                      <>
                        <Select value={order.driverId || ''} onValueChange={v => assignDriver(order.id, v)}>
                          <SelectTrigger className="w-48"><SelectValue placeholder={t.deliveryPage.assignDriver} /></SelectTrigger>
                        ...
                        <Button size="sm" disabled={!order.driverId} onClick={() => updateStatus(order.id, 'out_for_delivery')} className="gap-1">
                          <Truck className="w-3 h-3" /> {t.deliveryPage.dispatch}
                        </Button>
                      </>
                    )}
                    {order.status === 'out_for_delivery' && (
                      <Button size="sm" className="gap-1 bg-status-safe text-status-safe-foreground hover:bg-status-safe/90" onClick={() => updateStatus(order.id, 'delivered')}>
                        <CheckCircle className="w-3 h-3" /> {t.deliveryPage.confirmDelivered}
                      </Button>
                    )}
                    {order.status === 'delivered' && (
                      <span className="text-xs text-status-safe font-medium flex items-center gap-1"><CheckCircle className="w-3 h-3" /> {t.deliveryPage.delivered}</span>
                    )}
                  </div>
                </CardContent>
              </Card>
